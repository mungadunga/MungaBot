module.exports = {
    ...
    execute(message, args){
        ...
        if(user){
            ...
            if(a <= 5){ //done
                setTimeout(function(){ 
                    message.channel.send(self + " rushes towards " + user.displayName + "!"); 
                }, 2000);

                if(b <= 5){ //done
                    setTimeout(function(){ 
                        message.channel.send(self + " uses his fist to deal physical damage against " + user.displayName + "!"); 
                    }, 4000);

                    if(c <= 5){ //done
                        setTimeout(function(){ 
                            message.channel.send(user.displayName + "uses his shield to block the attack!"); 
                        }, 6000);
